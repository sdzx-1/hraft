...
runFollow
  :: forall s n
   . (Monad n, MonadTime n, MonadTimer n, MonadFork n)
  => HState s n
  -> HEnv s n
  -> StdGen
  -> n (HState s n, (StdGen, ()))
...
follower
  ...
  => m ()
follower = do
  HEnv { persistentFun = PersistentFun { readCurrentTerm, writeCurrentTermAndVotedFor }, peersRecvQueue } <-
    R.ask @HEnv
            ...
            then do
              ...
              case res' of
                ...
                Left LastLogIndexGTSyncingParams -> do
                  ...
                  put
                    (SState { syncingPrevLogIndex = newPreIndex
                            , syncingLogLength    = 1
                            }
                    )
...
appendAction
  ...
  -> AppendEntries s
  -> m ()
appendAction ranNum peerNodeId AppendEntries { prevLogIndex, prevLogTerm, entries, leaderCommit }
  = do
...
voteAction
  ...
  -> RequestVote
  -> m ()
voteAction peerNodeId RequestVote { candidateId, lastLogIndex, lastLogTerm } =
  do
