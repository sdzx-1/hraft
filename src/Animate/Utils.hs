...
data NodeRestart = NodeRestart [NodeId] DiffTime
  deriving (Eq, Generic)

instance Show NodeRestart where
  show (NodeRestart ids dt) =
    "====================================================\n"
      ++ "|| "
      ++ " NodeRestart "
      ++ show ids
...
data NTracer a
  ...
  | N4 (TimeWrapper NetworkChange)
  | N5 (TimeWrapper NodeRestart)
