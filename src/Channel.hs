...
channelEffect
  ...
  -> Channel m a
  -> Channel m a
channelEffect beforeSend afterRecv Channel { send, recv } = Channel
  { send = \x -> do
             beforeSend x
             send x
  , recv = do
...
loggingChannel ident Channel { send, recv } = Channel { send = loggingSend
                                                      , recv = loggingRecv
                                                      }
 where
  loggingSend a = do
...
socketAsChannel sock = Channel { .. }
 where
  ...
  recv = mfilter (not . LBS.null) . pure <$> Socket.recv
    sock
...
toLazyByteString = BS.toLazyByteStringWith strategy LBS.empty
  where strategy = BS.untrimmedStrategy 800 LBS.smallChunkSize
...
convertCborDecoderLBS cborDecode sendMST = go []
